...
export function drawLinkSvg(svg, r) {
  ...
  const pathLen = Math.min(165, 72 + r.rangeKm * 0.17);
  ...
  const handsetLabelY = Math.min(phoneY + 26, groundY - 10);
  const elevLabelX = phoneX + 46;
  const showElevLabel = elevLabelX < SVG_W - 52;
  ...
  svg.innerHTML = `
    ...
    <path d="M 20 ${groundY} Q ${cx} ${groundY + 24} ${SVG_W - 20} ${groundY}" fill="none" stroke="${COL.earth}" stroke-width="2.5"/>
    <text x="28" y="${groundY - 6}" font-size="9" fill="${COL.muted}" dominant-baseline="auto">Earth surface</text>
    ...
    <text x="${midX}" y="${midY + 4}" text-anchor="middle" font-size="9" fill="${COL.accent}" font-weight="700" dominant-baseline="middle">d=${r.rangeKm.toFixed(0)} km</text>
    ...
    <text x="${satX}" y="${satY + 28}" text-anchor="middle" font-size="10" fill="${COL.ink}" font-weight="600">${r.blockLabel}</text>
    <text x="${satX}" y="${satY + 40}" text-anchor="middle" font-size="9" fill="${COL.muted}">h=${r.altKm} km · ${r.arraySqM} m²</text>
    ...
    <text x="${phoneX}" y="${handsetLabelY}" text-anchor="middle" font-size="10" fill="${COL.ink}">Handset · ε=${r.elevDeg}°</text>
    <path d="M ${arcStartX} ${phoneY} A ${arcR} ${arcR} 0 0 0 ${arcEndX} ${arcEndY}" fill="none" stroke="${COL.warn}" stroke-width="1.8"/>
    ${showElevLabel ? `<text x="${elevLabelX}" y="${phoneY - 6}" font-size="8" fill="${COL.warn}">ε</text>` : ""}
    <line x1="${satX}" y1="${satY + blockH + 4}" x2="${phoneX}" y2="${phoneY - 8}" stroke="${COL.accent}" stroke-width="1" opacity=".35"/>
    <rect x="12" y="${summaryY}" width="${SVG_W - 24}" height="58" rx="8" fill="#fff" stroke="${COL.line}"/>
    <text x="${cx}" y="${summaryY + 16}" text-anchor="middle" font-size="10" fill="${COL.ink}" font-weight="600">Margin ${r.marginDb.toFixed(1)} dB · ${r.linkOk ? "Link closes" : "Below sensitivity"}</text>
    <text x="${cx}" y="${summaryY + 30}" text-anchor="middle" font-size="9" fill="${COL.muted}">λ=${(r.lambdaM * 100).toFixed(1)} cm · Friis PL=20 log₁₀(4πd/λ)=${r.pathLossDb.toFixed(1)} dB</text>
    <text x="${cx}" y="${summaryY + 44}" text-anchor="middle" font-size="9" fill="${COL.muted}">P_rx = EIRP + G_tx − PL − L_impl − L_atm → ${r.rxDbm.toFixed(1)} dBm</text>`;
}
