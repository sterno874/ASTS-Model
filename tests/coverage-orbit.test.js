...
import { overlapCoverageFraction, footprintAreaFraction, constellationTimeline, computeCoverageOrbit, US_COVERAGE_SOURCES, overlapHeuristicExplain, formatTimelineMonth } from "../js/math/coverage-orbit.js";
...
test("overlap uses probabilistic union not linear f×N", () => {
  const f = footprintAreaFraction();
  const at5 = overlapCoverageFraction(5);
  const linearCapped = Math.min(1, f * 5);
  assert.ok(at5 < linearCapped);
  assert.ok(at5 > f * 2);
});
